...
| Sliding Window | `TextSlidingWindow` | `ids`（`TENSOR`，1 维流） | `window` INT 4 (1~4096) | 把 token 流切成（上下文，下一 token）样本对——语言模型的下一词预测数据集；输出 `x` `(samples, window)` 与 `y` `(samples,)`。上报实时进度（每切一个样本更新一次），长语料不再看起来卡死 |
