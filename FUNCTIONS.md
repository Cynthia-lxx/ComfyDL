...
| Sliding Window | `TextSlidingWindow` | `ids` (`TENSOR`, 1-D stream) | `window` INT 4 (1~4096) | Cuts the stream into `(context, next-token)` pairs — the next-token dataset of a language model; outputs `x` `(samples, window)` and `y` `(samples,)`. Reports live progress (one bar update per sample), so cutting a long corpus no longer looks frozen |
